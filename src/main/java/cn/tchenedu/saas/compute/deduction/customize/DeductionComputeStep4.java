 ...
 *
 * 第四步,进行行政班确定的情况下,计算学考版分班情况
 */
public class DeductionComputeStep4 {
    ...
    public StepResponseDto compute(StepResponseDto request, int maxNumber) {
        ...
        for(Integer key : rsAll.keySet()){
            ...
            for (DeductionComposeInfoDto dto : deductionComposeInfoDtos) {
                ...
                List<ClassDeduction> xkClasses = response.getXuekClassComposeList().get(key);
                if (xkClasses == null) {
                    xkClasses = new ArrayList<>();
                    xkClasses.add(item);
                    response.getXuekClassComposeList().put(key, xkClasses);
                } else {
                    xkClasses.add(item);
                }
            }
